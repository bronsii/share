...
const privacyCopy = {
  de: {
    ...
    localBody: "Die Seite verwendet keine Werbe- oder Analyse-Tracker. Der Browser speichert die Sprachauswahl und bei Bedarf tablokale Informationen zur Upload-Wiederaufnahme, einschließlich Schlüsselmaterial und separater Löschberechtigung. Nach jedem Upload kannst du freiwillig genau diese Freigabe auf dem Gerät merken: höchstens fünf Freigabe- und private Löschlinks inklusive Schlüssel, Speicher- und Ablaufdatum in localStorage, ohne Dateien oder Dateinamen. Diese Liste wird nicht an den Server übertragen oder synchronisiert. Andere mit Zugriff auf das Browserprofil können damit Dateien öffnen und löschen; nutze die Funktion nicht auf gemeinsam genutzten Geräten. Du kannst Einträge jederzeit nur lokal entfernen. Abgelaufene Einträge werden beim nächsten Seitenbesuch oder während der Nutzung entfernt; ohne Seitenbesuch kann der Browserspeicher länger bestehen bleiben. Die separate, bestätigte Serverlöschung nutzt den privaten Löschlink. QR-Code und Sammel-ZIP entstehen lokal auf deinem Gerät. Bei einer Anmeldung in der Verwaltung kommt ein notwendiges, auf zwei Stunden begrenztes Sitzungscookie hinzu.",
  ...
  en: {
    ...
    localBody: "The site does not use advertising or analytics trackers. The browser stores your language choice and, when needed, tab-local upload-resume information, including key material and the separate deletion credential. After each upload, you may choose to remember that specific share on this device: up to five share and private deletion links including keys, save dates and expiry dates in localStorage, without files or file names. This list is not sent to the server or synced. Others with access to the browser profile can open and delete these files; do not use this feature on shared devices. Entries can be removed locally at any time. Expired entries are removed on your next visit or while the page is in use; browser storage may persist longer if you do not revisit. Separate, confirmed server deletion uses the private deletion link. QR codes and combined ZIP archives are created locally on your device. Signing in to the administration area additionally sets a necessary session cookie limited to two hours.",
    sourceTitle: "Open source",
